 ...
 *  ADATTO ANCHE A LAMPADE LED NON DIMMERABILI
...
void loop()
{
  ...
  //
  if (ciclopwm < 30) delay(50);
  if (ciclopwm < 10) delay(100);
  if (ciclopwm < 5) delay(200);
  if (ciclopwm < 3) delay(500);
 // if (millis() > timer) ciclopwm = 0;    // timer opzionale:
  analogWrite(caricoPin, ciclopwm);
  //
  // lcd.setCursor(0, 0);                // attivare solo per test:
  // lcd.print("dutycycle:      ");      // attivare solo per test:
  // lcd.setCursor(11, 0);               // attivare solo per test:
  // lcd.print(ciclopwm);                // attivare solo per test:
  // lcd.setCursor(0, 1);                // attivare solo per test:
  // lcd.print("segmenti.:      ");      // attivare solo per test:
  // lcd.setCursor(11, 1);               // attivare solo per test:
  // lcd.print(segmenti);                // attivare solo per test:
